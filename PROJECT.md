...
**ANALYTICS PIPELINE (2026-09-22, in progress):** `analytics/` — closed loop
chatbot/shop → Azure Data Lake Gen2 (`stuitchatbotlake6435`, container `lake`,
eastasia) → Snowflake `NORTHLIGHT_DW` BRONZE → SILVER → GOLD → MongoDB `insights`
→ `/admin/insights` page. Done: synthetic 12-month history (`simulate.py`, all
records `is_simulated`), app export with masking, both uploaded to the lake;
SQL for all layers (`analytics/snowflake/01–03`); runner `pipeline.py`; insights
endpoint and page. **Waiting on the developer:** run
`analytics/snowflake/00_admin_setup.sql` as ACCOUNTADMIN in Snowsight and put the
token into `analytics/.env` as `SNOWFLAKE_PAT` (that script also revokes the token
exposed in chat). Then `analytics/.venv/bin/python analytics/pipeline.py snowflake`
and `... feedback`. Still to do after that: report section and demo step for the
analytics (requested), redeploy backend + frontend for the insights page.

**Report:** `Code/BaoCao_DoAn_QTDA_Chatbot.docx` (outside the repo), generated by
scripts in the session scratchpad. Roles as the developer described them: first
PM + idea/BA = [Thành viên 4] (then unreachable; PM handed over = CR00); seminar =
project task 1.1.4 by [Thành viên 2], [Thành viên 3] before they left; the
developer = architect and main developer, PM after the handover. Contribution by
estimated WBS hours (158 h): 86.6 / 5.1 / 5.1 / 3.2 %. PM tool recommended: Jira
Free (`Code/jira_backlog.csv` for import). Demo guide: `Code/DEMO_GUIDE.md`.
