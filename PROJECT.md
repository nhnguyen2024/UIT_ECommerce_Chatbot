...
`NORTHLIGHT_DW` (account on Azure Southeast Asia, role `NL_PIPELINE`, warehouse
`NL_WH`, service user `CHATBOT_SVC` with token in `analytics/.env`) BRONZE (127,786
...
**PM report (current):** `Code/BaoCao_QLDA_KeHoachDuAn_Chatbot.docx` — follows the
structure of `Code/ref/Kế hoạch dự án (CheckMeIn)_Final_vP.pdf` plus chapters 4–5
for the doc_requirement items. Approach: SDLC = Scrum, project frame tailored with
predictive tools (deliverable WBS, milestones, CPM, cost baseline), justified by
the three required factors. Contribution by completed story points: 90.3 / 3.5 /
3.5 / 2.7 %. The older `Code/BaoCao_DoAn_QTDA_Chatbot.docx` is superseded.
**Report generator history:** `Code/BaoCao_DoAn_QTDA_Chatbot.docx` (outside the repo), generated by
